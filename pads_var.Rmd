...
subtitle: "Financial Analytics - Programa Avançado em Data Science - Insper"
author: "Viviane Sanchez"
date: "31/10/2020 - 13/11/2020"
output: 
  html_document:
    toc: true
    toc_float:
...
rmarkdown::includes(in_header = NULL, before_body = NULL, after_body = NULL)
...
A seguir são feitas as análises gráficas dos preços e retornos para **duas** ações. Os testes estatísticos necessários para verificar a presença de raiz unitária, normalidade, autocorrelação e efeitos ARCH serão no item seguinte.
...
O teste de raiz unitária é feito com a função `adf.test` para verificar a estacionaridade da série temporal. No caso, se houver raiz unitária, a série *não* é estacionária.
...
Para verificar se os retornos são autocorrelacionados, é feito um teste Ljung-Box nas séries de quadrados dos retornos das duas ações com a função `Box.test`
...
Para verificar a presença de efeitos ARCH, é feito o teste de Lagrange nas séries de quadrado dos retornos com a função `archtest`
...
rownames(garch_fit_1@fit$matcoef)

coefs <- as_tibble(garch_fit_1@fit$matcoef) %>% 
  janitor::clean_names() %>% 
  bind_cols(param = rownames(garch_fit_1@fit$matcoef)) %>% 
...
garch_params_1 <- reactive({
  ...
  isolate({
    ...
    as_tibble(garch_1@fit$matcoef) %>% 
          janitor::clean_names() %>% 
          bind_cols(param = rownames(garch_1@fit$matcoef)) %>% 
          select(param, everything())
...
garch_params_2 <- reactive({
  ...
  isolate({
    ...
    as_tibble(garch_2@fit$matcoef) %>% 
          janitor::clean_names() %>% 
          bind_cols(param = rownames(garch_2@fit$matcoef)) %>% 
          select(param, everything())
  })
...
isolate({
  ...
  a_b1 <- garch_params_1() %>% 
    filter(param %in% c("alpha1", "beta1")) %>% 
    summarise(total = sum(estimate)) %>% 
    pull(total)
  
  omg1 <- garch_params_1() %>% 
    filter(param %in% c("omega")) %>% 
    summarise(total = sum(estimate)) %>% 
    pull(total)
  
  bic1 <- infocriteria(garch_1)[2]
  bic2 <- infocriteria(garch_2)[2]
...
Pelos parâmetros dos modelos ajustados, nota-se que a volatilidade da ação ECOR3 foi melhor ajustada do que da ação VALE3, por possuir um BIC menor em módulo. Outros modelos poderiam ser testados para otimizar e avaliar o BIC ou outro crtíerio de informação para encontrar o modelo com ordens mais adequdas.

# VaR

Abaixo, são selecionados os parâmetros para cálculo do VaR de ambas as ações pela abordagem econométrica, visto que foi confirmada a não normalidade dos retornos.
...
var_doc <- reactive({round(value_at_risk(), digits = 2)*100})

```

O VaR de `r days_ahead` dia(s) para a ação é de `r var_doc`% e a perda total esperada no período para uma posição de R$ `r position` é de R$ `r loss` 
...
value_at_risk_2 <- reactive({
  ...
  abs(r_2() + tc_2()*sigma_2()/sqrt((v_2()/(v_2()-2))))
...
var_doc_2 <- reactive({round(value_at_risk_2(), digits = 2)*100})

```

O VaR de `r days_ahead` dia(s) para a ação é de `r var_doc_2`% e a perda total esperada no período para uma posição de R$ `r position` é de R$ `r loss_2`.
...
var_port_doc <-  reactive({round(port_var(), digits = 2)*100})

```

O VaR da carteira é de `r var_port_doc`% e a perda esperada de R$ `r port_loss`
...
- [Ibovespa]((http://www.b3.com.br/pt_br/market-data-e-indices/indices/indices-amplos/indice-ibovespa-ibovespa-composicao-da-carteira.htm))
- Uribe, P. Material da aula Financial Analytics
...
- IV - Alexander, C. Market Risk Analysis - Value at Risk Models
