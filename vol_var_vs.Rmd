...
position <- reactive({isolate({input$position})})

days_ahead <- reactive({isolate({input$days_ahead})})
...
O VaR de `r days_ahead` dia(s) para a ação é de `r value_at_risk` e a perda total esperada no período para uma posição de R$ `r position` é de `r loss` reais
...
O VaR de `r days_ahead` dia(s) para a ação é de `r value_at_risk_2` e a perda total esperada no período para uma posição de R$ `r position` é de `r loss_2` reais.
