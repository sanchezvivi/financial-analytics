...
tibble(param = c("ar1", "ma1", "ma2", "omega", "alpha", "beta1", "shape"),
       estimate = map(param, ~garch_fit_1@fit$matcoef[param,][1]),
       error = garch_fit_1@fit$matcoef[param,][2])
...
isolate({
  ...
  omg1 <- garch_params_1() %>% filter(param == "omega") %>% pull(estimate)
  ...
  omg2 <- garch_params_2() %>% filter(param == "omega") %>% pull(estimate)
  
  
  tibble(ticker = c(ticker_1(), ticker_2()),
         a_b = c(a_b1, a_b2),
         omg = c(omg1, omg2),
         om_ab = omg/(1-a_b))
